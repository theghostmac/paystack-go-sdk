...
// FetchBankProviders fetches the available bank providers for a dedicated virtual account.
// It takes the client as an argument and returns the list of bank providers.
func FetchBankProviders(client *paystack_client.Client) (*FetchBankProvidersResponse, error) {
	...
	// Decode the response body
	var respData FetchBankProvidersResponse
	if err := json.NewDecoder(resp.Body).Decode(&respData); err != nil {
		return nil, fmt.Errorf("failed to decode response body: %w", err)
	}
